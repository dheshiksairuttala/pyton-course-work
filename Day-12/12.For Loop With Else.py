...
#● If there was even one unread notification (1), the loop would stop, and else wouldn't run.
...
#● If OTP is not correct after 3 attempts, the else block triggers to notify that the OTPhas expired.
...
#Explanation:
#The loop iterates through the list. When the target number is found, the break statement
...
#Explanation:
#The loop checks each number from 1 to 9. If a number is even, the continue statement
#skips printing and goes to the next iteration.
